...
webrtc.controller('videoController', ['$scope', '$window', '$location', '$http', 'EventSnap', 'EventCall', 'EventScreen', 'EventWhiteboard', 'awsApiGatewayFactory',
    function($scope, $window, $location, $http, EventSnap, EventCall, EventScreen, EventWhiteboard, awsApiGatewayFactory) {
        ...
        $scope.start = function(parameters, online) {
            ...
            // instructor default is one. if have any in the room, then set to false
            // as well is presenter.
            // once you have room then you can download parameter from cloud
            ...
            $scope.noVideo_ = $(UI_CONSTANTS.noVideo);
            ...
            $scope.fullscreenIconSet_ = new IconSet_(UI_CONSTANTS.fullscreenSvg);
            $scope.shareScreenIconSet_ = new IconSet_(UI_CONSTANTS.screenShareSvg);

            // check if device has screen share capability
            if (!navigator.getDisplayMedia && !navigator.mediaDevices.getDisplayMedia && $scope.presenter_ !== $scope.user) {
			...
			// open signalchannel shared by all channels
        ...
        function onScreenOff_() {
            ...
            $scope.screenCalls_ = [];
            $scope.screenStream_ = null;
            $scope.isScreenSharing_ = false;           
            $scope.localVideo_.srcObject = $scope.localStream_;
            if (inWhiteboard_) {
                return;
            }
            if ($scope.presenter_ === $scope.user) {         
                ...
                show_($scope.shareScreenIconSet_.iconElement);
		...
		function changeDevice() {
            ...
            if (typeof mediaConstraints.video === "object") {
				mediaConstraints.video.sourceId = $scope.selectedVideoDeviceId_;   
            }
			if (typeof mediaConstraints.audio === "object") {
				mediaConstraints.audio.sourceId = $scope.selectedPlaybackDeviceId_;
            }
        ...
        // function pushCallNavigation_(roomId, roomLink) {
        //     if (!isChromeApp()) {
        ...
        function setupUi_() {
            ...
            $(UI_CONSTANTS.screenShareSvg).onclick = toggleScreenSharing_.bind(this);
        ...
        function toggleScreenSharing_() {
            if (!$scope.isScreenSharing_) {
				let displayPromise = maybeGetDisplayMedia_();
				// ice should be ready
				displayPromise.then(function() {
                    // disable the button to share screen
                    ...
                    if ($scope.presenter_ === $scope.user) {
                        for (let call of $scope.calls_) {
                            ...
                            setTimeout(()=>{
                                $scope.loadingParams_.isInitiator = "true";
                                let callScreen = createCall_(call.getPeer(), $scope.screenStream_, $scope.channel_, true);
                                callScreen.start($scope.room); 
                            }, 4000); 
                        }
                    }				
				}).catch(function(error) {
        ...
        function onRemoteHangup_(peer) {
            ...
            for (let i = 0; i < $scope.calls_.length; i ++) {
        ...
        function onRemoteSdpSet_(peer, hasRemoteVideo) {
            // if in whiteboard mode do not change 
            if (inWhiteboard_) {
                return;
            }
            if (hasRemoteVideo) {
    ...
    /*     function onRecvSignalingChannelMessage_(message){
            var messageObj = parseJSON(message);
            if (messageObj.peer != null) {
                $scope.calls_[peer].onRecvSignalingChannelMessage(messageObj.message);
            }
        } */
        ...
        function waitForRemoteVideo_(peer) {
            ...
            if (video.readyState >= 2) { // i.e. can play
                trace('Remote video started; currentTime: ' + $scope.remoteVideo_.currentTime);
                transitionToActive_();
            } else {
        ...
        function openScreenWindow() {
            trace("open screen sharing");
            window.open("screen.html", "ShareScreen", "scrollbars=no,status=yes,location=no");
        };
        ...
        // for presenter, directly get to canvas and send data
		...
		var requestMediaAndIceServers_ = function() {
			if (!$scope.localStream_) {
				if (!$scope.isScreenSharing) {
					...
					$scope.getMediaPromise_ = maybeGetDisplayMedia_();
				}
			}
...
/*             if (!$scope.localStream_ && $scope.online_) {
        ...
        // local stream added
        function onScreenStreamAdded_(stream) {
            trace('User has granted access to local media.');
/*             if (!$scope.localStream_ && $scope.online_) {
                for (var i = 0; i < $scope.peers_.length; i++){
                    var peer = $scope.peers_[i];
            ...
            $scope.screenStream_ = stream;
            attachLocalStream_(stream);
        };

        var onUserMediaError_ = function(error) {
            var errorMessage = 'Failed to get access to local media. Error name was ' +
              error.name + '. Continuing without sending a stream.';
            displayError_('getUserMedia error: ' + errorMessage);
            alert(errorMessage);
        };
		...
		// Asynchronously request display media if needed.
		var maybeGetDisplayMedia_ = function() {
			// mediaConstraints.audio and mediaConstraints.video could be objects, so
			// check '!=== false' instead of '=== true'.
			var displayPromise = null;
			if (!$scope.screenStream_) {
				if (navigator.getDisplayMedia || navigator.mediaDevices.getDisplayMedia) {
					if (navigator.mediaDevices.getDisplayMedia) {
						displayPromise = navigator.mediaDevices.getDisplayMedia({video: true}).then(
							stream => {
								trace('Got access to display media:\n' + '\'');
                                stream.getVideoTracks()[0].onended = onScreenOff_.bind(this);
								onScreenStreamAdded_(stream);
							},
							error => {
                                onDisplayMediaError_(error);
                                return Promise.reject(error);
							}
						);
					} else if (navigator.getDisplayMedia) {
						displayPromise = navigator.getDisplayMedia({video: true}).then(
							stream => {
								trace('Got access to display media:\n' + '\'');
                                stream.getVideoTracks()[0].onended = onScreenOff_.bind(this);
								onScreenStreamAdded_(stream);
                    ...
                    displayError_('Unable to acquire screen capture, please upgrade chrome or firefox');
                    return Promise.reject('Unable to acquire screen capture, please upgrade chrome or firefox');
				}
        ...
        var onDisplayMediaError_ = function(error) {
            var errorMessage = 'Failed to get access to screen media. Error name was ' +
              error.name + '. Continuing without sending a stream.';
            displayError_('getDisplayMedia error: ' + errorMessage);
            alert(errorMessage);
        };
        ...
        function onRemoteStreamAdded_(peer, stream) {
            ...
            if ($scope.isScreenSharing_ && $scope.user === $scope.presenter_ && !peer.endsWith("_screen")) {
                //toggle to screen share mode and initiate a new share screen to new user
                let call = findCall(peer);
                let callscreen = findCall(peer + "_screen"); 
                if (call && !callscreen) {
                    call.stopVideoStream();
                    call.sendSignalingMessage({
                        user: $scope.user,
                    ...
                    $scope.loadingParams_.isInitiator = "true";
                    var callScreen = createCall_(call.getPeer(), $scope.screenStream_, $scope.channel_, true);
                    setTimeout(()=>{
                        callScreen.start($scope.room); 
                    }, 4000);       
                }
        ...
        // local screen share stream added
        // function onScreenStreamAdded_(stream) {
        //     trace('User has granted access to screen media.');
        //     $scope.screenStream_ = stream;
        //     activate_($scope.shareScreenIconSet_.iconElement);
        // };
        // receive remote the screen share
        ...
        function attachScreenStream_() {
            trace('Attaching screen stream.');
            displayStatus_('');
        ...
        // function toggleScreenShare_(on) {
        //     if (on) {
        //         show_($scope.screenDiv_);
        //     } else{
        //         hide_($scope.screenDiv_);
        //     }
        // };
        ...
        function onIceConnectionStateChange_(state, peer) {

        };

        function onSignalingStateChange_(peer) {
